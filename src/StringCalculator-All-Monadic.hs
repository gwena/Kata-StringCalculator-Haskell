...
assertEq :: (Either ParseError Integer, Either String Integer, String) -> (Bool, String)
assertEq (x, y, msg) = (b, msg) 
                  where b = case x of -- should be cleaner
                              Right vx -> case y of
                                Right vy -> vx == vy
                                Left _ -> False
                              Left _ -> case y of
...
assertMsg :: (Bool, String) -> String
assertMsg (b, msg) = (if b then " . " else redify " *** FAIL *** ") ++ msg

redify st = "\27[31m" ++ st ++ "\27[0m"

runtest =  mapM_ (putStrLn . assertMsg . assertEq)
